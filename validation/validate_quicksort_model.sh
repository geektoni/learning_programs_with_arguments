...
elif [ $2 == "no-save-load-partition" ]; then
  actions=( "PARTITION_UPDATE" "PARTITION" "QUICKSORT_UPDATE" "QUICKSORT" )
else
...
do
  ...
  python validate_quicksorting.py --load-path $1 --operation ${action} --save-results --verbose --output-dir $3
done