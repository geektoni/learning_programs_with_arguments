...
class QuickSortListEnv(Environment):
    ...
    def __init__(self, length=10, max_length=10, encoding_dim=32, sample_from_errors_prob=0.3, hierarchy=True,
                 ...
                 recursive_version=False, expose_pointers_value=True, complete_programs=False,
        ...
        self.complete_programs = complete_programs
        ...
        if hierarchy:
            ...
            if without_partition_update:
                ...
                self.programs_library = programs_library_without_save_load_partition
            elif reduced_set:
            ...
            if self.complete_programs:
                self.programs_library = generate_total_possible_programs(self.programs_library)

            for idx, key in enumerate(sorted(list(self.programs_library.keys()))):
                self.programs_library[key]['index'] = idx

            # Possible availabe args
            ...
            if self.complete_programs:
                ...
                self.prog_to_func, self.prog_to_precondition, self.prog_to_postcondition, self.failed_executions_env \
                    = generate_total_possible_conditions(self.prog_to_func, self.prog_to_precondition, self.prog_to_postcondition, self.failed_executions_env, self.programs_library)
    ...
    def _swap(self, arguments):
        ...
        if total < 2:
            self.scratchpad_ints[[self.p3_pos, self.p2_pos]] = self.scratchpad_ints[[self.p2_pos, self.p3_pos]]
        else:
    ...
    def _swap_precondition(self):
        return self.p1_pos != self.p2_pos or self.p2_pos != self.p3_pos or self.p1_pos != self.p3_pos
    ...
    def _save_ptr(self, arguments):
        ...
        for element in zip(arguments, [self.p1_pos, self.p2_pos, self.p3_pos]):
            if element[0] == 1:
                ...
                break
        if not found:
            self.temp_variables[0] = self.p3_pos
    ...
    def _load_ptr(self, arguments):
        if self.temp_variables[0] != -1:
            self.p1_pos = self.temp_variables[0] if arguments[0] != 0 else self.p1_pos
            self.p2_pos = self.temp_variables[0] if arguments[1] != 0 else self.p2_pos
            self.p3_pos = self.temp_variables[0] if arguments[2] != 0 else self.p3_pos
            self.temp_variables[0] = -1

    def _load_ptr_precondition(self):
        return self.temp_variables[0] != -1

    def _decrease_ctr(self, arguments):
        if self.counter > 0:
            self.counter -= 1
